...
function formMessageLauch(id,name,type,title,content,price,thumbnail){
    ...
    if(type==="user"){
        ...
        //         let { product } = messages[i];  // 구조 분해 할당을 사용하여 상품 정보 추출
        //         console.log(message);
        //         console.log(product);
        //
        ...
        $.get(url + "/listProducts/" + userId + "/" + id, function(response) {
            ...
            for (let i = 0; i < products.length; i++) {
                if (products[i]['my'] == userId) {
                    productTemplateHTML += '<div id="child_message" class="d-flex justify-content-start mb-4">' +
                        '<div id="child_message" class="msg_cotainer">' +
        ...
        function saveData(data) {
            ...
            //         // 저장 성공 시 추가적인 처리를 수행할 수 있습니다.
            //     },
            //     error: function(error) {
            //         console.error("데이터 저장 실패", error);
            //         // 저장 실패 시 오류 처리를 수행할 수 있습니다.
            //     }
            // });
        }
